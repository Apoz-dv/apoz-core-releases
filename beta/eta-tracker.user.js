...
(function () {
  ...
  const PROVENANCE = [
    ...
    { file: "formulas/equipment.json", fact: "equipment.slotUpgrade.boostFormula" },
  ];
  ...
  function petRollsFromMerged(merged) {
    ...
    const boost = 1 + slotBoostPercent(slotLevels.combat);   // fraction, not percent
    ...
    if (typeof merged.potionEffect === 'number' && isFinite(merged.potionEffect)
      ...
      const uBoost = 1 + slotBoostPercent(slotLevels.utility);
